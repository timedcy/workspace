...
#else 存储当期选股以及备选股 
if(O==1){
  save(stock_pick,file=paste("Z:\\team4share\\ZDQH\\实盘策略\\多因子选股模块\\RData\\",poolname,percent,"-all-",mark,".RData",sep="")) 
}else{
  stock_pick<-stock_pick[length(stock_pick)]
  save(stock_pick,file=paste("Z:\\team4share\\实盘策略\\多因子选股模块\\RData\\",names(stock_pick),"-",poolname,percent,"-",mark,".RData",sep="")) 
  #save(stock_pick,file=paste("E:\\ZDQH\\实盘策略\\多因子选股模块\\RData\\",Sys.Date(),"-",poolname,percent,"-",mark,".RData",sep="")) 
  ...
  #filename<-paste("E:\\ZDQH\\实盘策略\\多因子选股模块\\数据集\\策略选股\\",Sys.Date(),poolname,percent,sep="")
