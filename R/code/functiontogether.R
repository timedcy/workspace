...
tradeday<-function(start,end){
  library("WindR", lib.loc="M:/Program Files/R/R-3.2.1/library")
  w.start()
...
tradeday2<-function(start,end,startday,num){
  library("WindR", lib.loc="M:/Program Files/R/R-3.2.1/library")
  w.start()
  date1<-w.tdays(start,end)$Data[,1]
  date1<-as.character(date1)
  date<-seq(as.Date(startday), by = "month", length.out = num)
  date<-as.character(date)
  trad<-NULL
  for (i in 1:length(date)) {
    a=min(date1[date1>=date[i]])
    trad=c(trad,a)
  }
  trad<-as.Date(trad)
  w.stop()
