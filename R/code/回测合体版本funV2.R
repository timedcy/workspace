Backtesthedge<-function(cap,Invr,beta,stock,fee.ratio1,fee.ratio2,tax.ratio,impact.ratio1,impact.ratio2,plusday){
  ...
  add<-hold.new[which(is.na(hold.new[,'amt'])==1 | hold.new[,'amt']<100),]
  addw<-0
  if (nrow(add)>0){
    addw<-sum(add[,'weight'])/(nrow(hold.new)-nrow(add))
  }
  hold.new[,'weight']<-hold.new[,'weight']+addw
  hold.new<-hold.new[which(hold.new[,'amt']>1),]
  CT<-hold.new
...
write.csv(caplist1,file=paste("Z:\\实盘策略\\仓位管理\\cap8005_0915.csv",sep=""))
write.csv(caplist1,file=paste("Z:\\实盘策略\\仓位管理\\cap30080_0915.csv",sep=""))
write.csv(caplist1,file=paste("Z:\\实盘策略\\仓位管理\\cap8005_0930.csv",sep=""))
write.csv(caplist1,file=paste("Z:\\实盘策略\\仓位管理\\cap30080_0930.csv",sep=""))
