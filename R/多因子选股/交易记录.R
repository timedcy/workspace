...
for( i in names(stock)){
  ...
  date<-trade_day_chr[which(trade_day_chr==i)+1]
  ...
  sqlbase<-paste("select datetime,trade_code, open,oi from hs300if where datetime='",date,"'")
...
tpn<-tpn[,c(1,2)]
filename<-paste("Z:\\多因子选股模块\\数据集\\",names(stock),"-zz800",sep="")
dir.create(filename)
write.csv(tpn,file=paste(filename,"\\",names(stock),"-stockpick.csv",sep=""),row.names=F)
