...
state1<-function(lasttrade,names){
  ...
  stockback<-read.csv(paste(filename,"/",lasttrade,"-",names,"-backup.csv",sep=""),stringsAsFactors=F);
  ...
  #浮动权重处理(只需要清单的处理)
  stock_afterdays<-reweight(stockrew,lasttrade,Sys.Date())
  write.csv(stock_afterdays,"1_afterdays.csv",row.names=F,quote = F,fileEncoding = "UTF-8")
  
  #清单以及备选清单的今日停牌处理
  stopstock2<-getstops(Sys.Date());
  stockrew2<- setweight(stockrew,stopstock2);
  #backuprew2<- setweight(backuprew,stopstock2);
  write.csv(stockrew2,file="2_wts.csv",row.names=F,quote = F,fileEncoding = "UTF-8")
  write.csv(backuprew,file="3_backwtsorigin.csv",row.names=F,quote = F,fileEncoding = "UTF-8")
  #write.csv(backuprew,"4_backwtsorigin.csv",row.names=F,quote = F)
  #交易清单生成
  list<-generatelist(stockrew2);
  write.csv(list,file=paste(Sys.Date(),"-",names,".csv",sep=""),row.names=F,quote = F,fileEncoding = "UTF-8")
}
...
state3<-function(names){
  ...
  write.csv(list,file=paste(names,"-",Sys.Date(),".csv",sep=""),row.names=F,quote = F,fileEncoding = "UTF-8")
  write.csv(stockrew,file=paste(filename,"\\","1_wts.csv",sep=""),row.names=F,quote = F,fileEncoding = "UTF-8")
  write.csv(stockrew,file=paste(filename,"\\","2_backwts.csv",sep=""),row.names=F,quote = F,fileEncoding = "UTF-8")
}
...
state4<-function(names){
  ...
  write.csv(list,file=paste(names,"-",Sys.Date(),".csv",sep=""),row.names=F,quote = F,fileEncoding = "UTF-8")
  write.csv(stock,file=paste(filename,"\\","1_wts.csv",sep=""),row.names=F,quote = F,fileEncoding = "UTF-8")
  write.csv(stockback,file=paste(filename,"\\","2_backwts.csv",sep=""),row.names=F,quote = F,fileEncoding = "UTF-8")
}
