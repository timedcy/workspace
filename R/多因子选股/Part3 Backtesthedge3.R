Backtesthedge3<-function(cap,Invr,beta,stock,fee.ratio1,fee.ratio2,tax.ratio,impact.ratio1,impact.ratio2,lastday,bili){
  ...
  trade_all<-sqlQuery(ch,'select distinct datetime from dailyprice')[,1]  ##选取所有的交易日
  trade_all<-as.character(trade_all)
  
  adjday<-names(stock)   ##调仓日的前一天
  adjday00<-as.Date(adjday,origin='1899-12-30')
  lastday<-lastday
  
  trad<-NULL            ##trad为真正的交易日
  ...
  ##period.a为采用沪深300指数对冲的区间
  ##period.b为采用沪深300股指期货对冲的区间
  ...
  ##定义需要返回的caplist
  caplist<-data.frame()
  caplist[1,'datetime']<-adjday[1]  ##caplist的第一日期为第一个调仓日的前一天
  caplist[1,'cap']<-cap
  caplist[1,'return']<-0
  caplist[1,'cash']<-cap
  ...
  if (trad[1]<'2010-04-17'){
    ...
    ##如果没有这个if语句的话，如果length(period.a)=1的话，会出现问题
  ...
  caplist[,"net value"]=caplist[,2]/caplist[1,2]
  caplist[,"position"]=1
