Backtesthedge3<-function(cap,Invr,beta,stock,fee.ratio1,fee.ratio2,tax.ratio,impact.ratio1,impact.ratio2,lastday){
  ...
  sql4<-paste("SELECT trade_code,volume,amt from winddata.dailyprice WHERE datetime ='",trad[1],"'")
  stock.volume<-sqlQuery(ch,sql4)
  sqln=paste("SELECT trade_code,avg(volume) from winddata.dailyprice where ","datetime<'",trad[1],"'","and datetime>='",as.Date(trad[1])-30,"'"," group by trade_code",sep="")
  avg_volume=sqlQuery(ch,sqln);
  colnames(avg_volume)=c("trade_code","avg_volume")
  hold.new<-merge(hold.new,stock.volume,by='trade_code')  
  hold.new<-merge(hold.new,avg_volume,by='trade_code')  
  add<-hold.new[which(is.na(hold.new[,'volume'])==1 | hold.new[,'volume']<hold.new[,"avg_volume"]),]
  addw<-0
  if (nrow(add)>0){
    addw<-sum(add[,'weight'])/(nrow(hold.new)-nrow(add))
  }
  hold.new[,'weight']<-hold.new[,'weight']+addw
  hold.new<-hold.new[which(hold.new[,'volume']>=hold.new[,"avg_volume"]),]
  ...
  if (trad[1]<'2010-04-17'){
    ...
    for ( d in 2:length(period.a)){
      ...
      if (is.element(period.a[d],trad)==1){   
        ...
        sql4<-paste("SELECT trade_code,volume,amt from winddata.dailyprice WHERE datetime ='",period.a[d],"'")
        stock.volume<-sqlQuery(ch,sql4)
        sqln=paste("SELECT trade_code,avg(volume) from winddata.dailyprice where ","datetime<'",period.a[d],"'","and datetime>='",as.Date(period.a[d])-30,"'"," group by trade_code",sep="")
        avg_volume=sqlQuery(ch,sqln);
        colnames(avg_volume)=c("trade_code","avg_volume")
        hold.new<-merge(hold.new,stock.volume,by='trade_code')  
        hold.new<-merge(hold.new,avg_volume,by='trade_code')  
    ...
    if ( length(period.b)>=1){
      ...
      for ( d in 1:length(period.b)){
        ...
        if (is.element(period.b[d],trad)==1){   
          ...
          sql4<-paste("SELECT trade_code,volume,amt from winddata.dailyprice WHERE datetime ='",period.b[d],"'")
          stock.volume<-sqlQuery(ch,sql4)
          sqln=paste("SELECT trade_code,avg(volume) from winddata.dailyprice where ","datetime<'",period.b[d],"'","and datetime>='",as.Date(period.b[d])-30,"'"," group by trade_code",sep="")
          avg_volume=sqlQuery(ch,sqln);
          colnames(avg_volume)=c("trade_code","avg_volume")
          hold.new<-merge(hold.new,stock.volume,by='trade_code')  
          hold.new<-merge(hold.new,avg_volume,by='trade_code')  
