...
#Author:
#       wrote by wwl
#       update by wang gaobin<313085945@qq.com>
#File description:Backtest the portfolio we constructed with hedge

Backtesthedge4<-function(cap,Invr,beta,stock,fee.ratio1,fee.ratio2,tax.ratio,impact.ratio1,impact.ratio2,lastday,bili,hs300if){
  
  library(RODBC)  
  ch<-odbcConnect('winddata',uid='root',pwd='123')
  #ch<-odbcConnect('mydata',uid='root',pwd='123')
  ...
  sql1<-paste("SELECT trade_code,open from dailyprice WHERE datetime ='",trad[1],"'")
  ...
  sql4<-paste("SELECT trade_code,amt from dailyprice WHERE datetime ='",trad[1],"'")
  stock.amt<-sqlQuery(ch,sql4)
  sqln=paste("SELECT trade_code,avg(amt) from dailyprice where ","datetime<'",trad[1],"'","and datetime>='",as.Date(trad[1])-90,"'"," group by trade_code",sep="")
  ...
  if (trad[1]<'2010-04-17'){
    ...
    for ( d in 2:length(period.a)){
      ...
      sql1<-paste("SELECT trade_code,open from dailyprice WHERE datetime ='",period.a[d],"'")
      stock.open<-sqlQuery(ch,sql1)
      ...
      if (is.element(period.a[d],trad)==1){   
        ...
        sql4<-paste("SELECT trade_code,amt from dailyprice WHERE datetime ='",period.a[d],"'")
        stock.amt<-sqlQuery(ch,sql4)
        sqln=paste("SELECT trade_code,avg(amt) from dailyprice where ","datetime<'",period.a[d],"'","and datetime>='",as.Date(period.a[d])-90,"'"," group by trade_code",sep="")
        avg_amt=sqlQuery(ch,sqln);
        colnames(avg_amt)=c("trade_code","avg_amt")
        hold.new<-merge(hold.new,stock.amt,by='trade_code')  
        hold.new<-merge(hold.new,avg_amt,by='trade_code')  
